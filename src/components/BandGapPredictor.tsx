...
export function BandGapPredictor() {
  ...
  const isGuestLimitReached = !userEmail && status !== 'loading' && predictionCount >= 2;
  ...
  const loadHistory = async () => {
    ...
    try {
      ...
      setHistory(rows.map((row) => ({
        id: row.id,
        text: row.input_text,
        prediction: row.output.band_gap_ev,
        unit: 'eV',
        elapsedMs: 0,
  ...
  const handlePredict = async () => {
    ...
    if (isGuest && predictionCount >= guestLimit) {
    ...
    try {
      ...
      if (isGuest) {
        updatePredictionCount(Math.min(guestLimit, predictionCount + 1));
      } else {
        await loadHistory();
      }
              ...
              {history.length > 0 ? history.map((entry, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-2 text-[11px] text-slate-500">
                      {entry.elapsedMs > 0 ? `${entry.elapsedMs.toFixed(0)} ms` : entry.timestamp.toLocaleString()}
                    </div>
                  </button>
                );
              }) : (
                <div className="rounded-xl border border-dashed border-white/10 bg-white/5 px-4 py-8 text-center">
                  <p className="text-sm text-slate-500">
                    {historyLoading ? 'Loading saved predictions...' : 'Saved predictions will appear here after you run the model.'}
                  </p>
                </div>
              )}
            </div>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <p className="max-w-2xl text-base leading-relaxed text-slate-400 md:text-lg">
                    Predict material properties using AI-powered models.
                  </p>
                </div>
          ...
          <div className="grid gap-6 2xl:grid-cols-[minmax(0,1.3fr)_420px]">
            <div className="space-y-6">
              <Card className="p-6 border-white/10 bg-white/5 backdrop-blur-xl shadow-xl">
                ...
                <div className="mt-4 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div className="space-y-2">
                    ...
                    {!userEmail && status !== 'loading' && (
                      <div className="rounded-2xl border border-yellow-500/20 bg-yellow-500/5 px-3 py-2 text-sm text-yellow-200">
                        You are using guest mode (<span className="font-semibold text-white">{Math.max(0, 2 - predictionCount)}</span> free predictions remaining)
                      </div>
                    )}
                  </div>
                  <Button 
                    ...
                    disabled={loading} 
                    className="gap-2 sm:min-w-44 bg-yellow-500 hover:bg-yellow-400 text-black font-semibold rounded-xl shadow-[0_0_20px_rgba(250,204,21,0.3)] hover:shadow-[0_0_30px_rgba(250,204,21,0.5)] transition-all duration-200 disabled:opacity-50 disabled:hover:bg-yellow-500"
                  >
                    {loading ? (
                      <>
                ...
                {isGuestLimitReached && (
                  <div className="mt-4 rounded-2xl border border-yellow-500/30 bg-slate-950/90 px-4 py-4 text-sm text-slate-100 shadow-sm">
                    <div className="flex items-start gap-3">
                      <span className="mt-1 text-xl">⚠</span>
                      <div className="space-y-2">
                        <div className="font-semibold text-white">Free limit reached</div>
                        <div className="text-slate-400">Login to continue with unlimited predictions.</div>
                        <Button
                          type="button"
                          onClick={handleContinueToLogin}
                          className="inline-flex items-center rounded-full bg-yellow-500 px-4 py-2 text-sm font-semibold text-black transition hover:bg-yellow-400"
                        >
                          Continue
                        </Button>
                      </div>
